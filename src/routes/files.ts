...
export function createFilesRouter(deps: Dependencies): Router {
  ...
  router.get("/files/:id/download", (req, res) => {
    ...
    if (file.user_id !== current.user.id && !hasRole(current, "support", "admin")) {
      sendErrorPage(res, 404, "File Not Found", "We couldn't find that file.");
      return;
    }
  ...
  router.get("/files/:id/signed-download", (req, res) => {
    ...
    if (!Number.isSafeInteger(fileId) || !verifySignedDownload(deps.downloadSigningKey, fileId, expires, signature)) {
      sendErrorPage(res, 403, "Download Link Unavailable", "This download link is invalid or has expired.");
      return;
    }
